...
BASE_DIR="$HOME/intercom-suite"
BIN_DIR="$BASE_DIR/bin"
CFG_DIR="$HOME/.config/intercom-suite"

mkdir -p "$BIN_DIR" "$CFG_DIR"

# ----- default configs -----
[ -f "$CFG_DIR/wallets.json" ] || cat > "$CFG_DIR/wallets.json" <<'JSON'
{
  "wallets": []
}
JSON

[ -f "$CFG_DIR/rpc.json" ] || cat > "$CFG_DIR/rpc.json" <<'JSON'
{
...
# ----- wallet_tracker.js -----
cat > "$BIN_DIR/wallet_tracker.js" <<'JS'
#!/usr/bin/env node
"use strict";
...
npm init -y >/dev/null 2>&1 || true
npm install axios inquirer@8 >/dev/null 2>&1
chmod +x "$BIN_DIR/wallet_tracker.js" "$BIN_DIR/swap_link.js"

echo
echo "[OK] Installed!"
echo "Run:  $BASE_DIR/menu.sh"
